...
def generate_anniversary_outfit_task(
    ...
    db = SessionLocal()
    try:
        from app.services.anniversary_service import generate_anniversary_outfit
        from app.schemas.outfit import AnniversaryOutfitRequest

        logger.info(f"Generating anniversary outfit for customer {customer_id}")
        request = AnniversaryOutfitRequest(
            customer_id=customer_id,
            days_window=days_window,
            send_email=bool(recipient_email),
            recipient_email=recipient_email,
        )
        result = generate_anniversary_outfit(db, store_id, request)
        logger.info(f"Anniversary outfit {result.id} generated, status={result.status}")
        return {"outfit_id": result.id, "status": result.status}
    except Exception as exc:
